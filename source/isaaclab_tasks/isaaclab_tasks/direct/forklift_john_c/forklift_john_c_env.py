...
from isaaclab_assets.robots.forklift_john_c import FORKLIFT_JOHN_C_CFG
...
class ForkliftEnvCfg(DirectRLEnvCfg):
    ...
    robot_cfg: ArticulationCfg = FORKLIFT_JOHN_C_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class ForkliftEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ForkliftEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._throttle_dof_idx, _ = self.forklift_john_c.find_joints(self.cfg.throttle_dof_name)
        self._steering_dof_idx, _ = self.forklift_john_c.find_joints(self.cfg.steering_dof_name)
        ...
        if lift_testing:
            ...
            self._lift_dof_idx, _ = self.forklift_john_c.find_joints(self.cfg.lift_dof_name)
    ...
    def _setup_scene(self):
        ...
        self.forklift_john_c = Articulation(self.cfg.robot_cfg)
        self.waypoints = VisualizationMarkers(self.cfg.waypoint_cfg)
        self.object_state = []
        
        self.scene.clone_environments(copy_from_source=False)
        self.scene.filter_collisions(global_prim_paths=[])
        self.scene.articulations["forklift_john_c"] = self.forklift_john_c
    ...
    def _apply_action(self) -> None:
        self.forklift_john_c.set_joint_velocity_target(self._throttle_action, joint_ids=self._throttle_dof_idx)
        self.forklift_john_c.set_joint_position_target(self._steering_state, joint_ids=self._steering_dof_idx)
        if lift_testing:
            self.forklift_john_c.set_joint_position_target(self._lift_state, joint_ids=self._lift_dof_idx)

    def _get_observations(self) -> dict:
        current_target_positions = self._target_positions[self.forklift_john_c._ALL_INDICES, self._target_index]
        self._position_error_vector = current_target_positions - self.forklift_john_c.data.root_pos_w[:, :2]
        self._previous_position_error = self._position_error.clone()
        self._position_error = torch.norm(self._position_error_vector, dim=-1)

        heading = self.forklift_john_c.data.heading_w
        target_heading_w = torch.atan2(
            self._target_positions[self.forklift_john_c._ALL_INDICES, self._target_index, 1] - self.forklift_john_c.data.root_link_pos_w[:, 1],
            self._target_positions[self.forklift_john_c._ALL_INDICES, self._target_index, 0] - self.forklift_john_c.data.root_link_pos_w[:, 0],
        )
        self.target_heading_error = torch.atan2(torch.sin(target_heading_w - heading), torch.cos(target_heading_w - heading))

        if lift_testing:
            lift_joint_pos = self.forklift_john_c.data.joint_pos[:, self._lift_dof_idx].squeeze(-1)
            lift_joint_vel = self.forklift_john_c.data.joint_vel[:, self._lift_dof_idx].squeeze(-1)

        if stop_testing:
            dist_to_stop = torch.norm(self._stop_sign_positions - self.forklift_john_c.data.root_pos_w[:, :2], dim=-1, keepdim=True)
        ...
        obs_parts = [
            ...
            self.forklift_john_c.data.root_lin_vel_b[:, 0].unsqueeze(dim=1),
            self.forklift_john_c.data.root_lin_vel_b[:, 1].unsqueeze(dim=1),
            self.forklift_john_c.data.root_ang_vel_w[:, 2].unsqueeze(dim=1),
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        fwd_dir = self.forklift_john_c.data.root_lin_vel_w[..., :2]  # Approximate forward direction
        fwd_dir = torch.nn.functional.normalize(fwd_dir, dim=-1)

        # From Leatherback
        env_ids = self.forklift_john_c._ALL_INDICES
        goal_pos = self._target_positions[env_ids, self._target_index]  # [num_envs, 2]
        to_wp = goal_pos - self.forklift_john_c.data.root_pos_w[..., :2]
        ...
        if stop_testing:
            ...
            forklift_pos = self.forklift_john_c.data.root_pos_w[:, :2]
            dist_to_stop = torch.norm(self._stop_sign_positions - forklift_pos, dim=-1)
            in_stop_zone = dist_to_stop < self.stop_zone

            lin_speed = torch.norm(self.forklift_john_c.data.root_lin_vel_b[:, :2], dim=-1)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        default_state = self.forklift_john_c.data.default_root_state[env_ids]
        forklift_john_c_pose = default_state[:, :7]
        forklift_john_c_velocities = default_state[:, 7:]
        joint_positions = self.forklift_john_c.data.default_joint_pos[env_ids]
        joint_velocities = self.forklift_john_c.data.default_joint_vel[env_ids]

        forklift_john_c_pose[:, :3] += self.scene.env_origins[env_ids]
        forklift_john_c_pose[:, 0] -= self.env_spacing / 2
        forklift_john_c_pose[:, 1] += 2.0 * torch.rand((num_reset), dtype=torch.float32, device=self.device) * self.course_width_coefficient

        angles = torch.pi / 6.0 * torch.rand((num_reset), dtype=torch.float32, device=self.device)
        forklift_john_c_pose[:, 3] = torch.cos(angles * 0.5)
        forklift_john_c_pose[:, 6] = torch.sin(angles * 0.5)

        self.forklift_john_c.write_root_pose_to_sim(forklift_john_c_pose, env_ids)
        self.forklift_john_c.write_root_velocity_to_sim(forklift_john_c_velocities, env_ids)
        self.forklift_john_c.write_joint_state_to_sim(joint_positions, joint_velocities, None, env_ids)
        ...
        current_target_positions = self._target_positions[self.forklift_john_c._ALL_INDICES, self._target_index]
        self._position_error_vector = current_target_positions[:, :2] - self.forklift_john_c.data.root_pos_w[:, :2]
        self._position_error = torch.norm(self._position_error_vector, dim=-1)
        self._previous_position_error = self._position_error.clone()

        heading = self.forklift_john_c.data.heading_w[:]
        target_heading_w = torch.atan2( 
            self._target_positions[:, 0, 1] - self.forklift_john_c.data.root_pos_w[:, 1],
            self._target_positions[:, 0, 0] - self.forklift_john_c.data.root_pos_w[:, 0],
        )
